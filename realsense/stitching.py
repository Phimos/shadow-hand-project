...
class PointCloudMatcher(object):
    ...
    def __init__(
        ...
        self.max_correspondence_distance_coarse = (
            max_correspondence_distance_coarse_scale * voxel_size
        )
        self.max_correspondence_distance_fine = (
            max_correspondence_distance_fine_scale * voxel_size
        )
    ...
    def full_registration(
        self, pcds: List[o3d.geometry.PointCloud]
    ...
    def fit(self, pcds: List[o3d.geometry.PointCloud]) -> None:
        ...
        # show optimized pose graph
        for i in range(len(pose_graph.nodes)):
            print(pose_graph.nodes[i].pose)
        for i in range(len(pose_graph.edges)):
            print(pose_graph.edges[i].transformation)

        extrinsics = []
        for i in range(len(pose_graph.nodes)):
            extrinsics.append(pose_graph.nodes[i].pose)
        print(extrinsics)

    def combine(
        self, pcds: List[o3d.geometry.PointCloud], downsample: bool = True
    ) -> o3d.geometry.PointCloud:
        combined = o3d.geometry.PointCloud()
        for point_id in range(len(pcds)):
            combined += deepcopy(pcds[point_id]).transform(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--calibration", type=str, default="calibration/calibration.json"
    )
    args = parser.parse_args()

    multiview = MultiViewSystem.from_workspace(Path(args.workspace))
    # if not multiview.valid_calibration():
    print("invalid calibration, load from config ...")
    multiview.load_calibration(Path(args.calibration))
    ...
    for name, camera in multiview.named_cameras:
        filepath_color = multiview.get_camera_color_dir(name) / f"{args.image}.png"
        filepath_depth = multiview.get_camera_depth_dir(name) / f"{args.image}.png"
        # print(name, camera)
        # filepath_depth = dataset_dir / name / "depth" / "1679464853796.png"
        # filepath_color = dataset_dir / name / "color" / "1679464853796.png"

        if not filepath_depth.exists() or not filepath_color.exists():
            continue
